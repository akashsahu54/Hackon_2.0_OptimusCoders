"""Workflow Execution Service — execute workflow actions based on triggers (MongoDB)."""

from typing import Dict, Any
from pymongo.database import Database


def execute_workflow(workflow: dict, document_id: str, db: Database) -> Dict[str, Any]:
    """
    Execute a workflow action for a given document.
    workflow: MongoDB workflow document (dict).
    Returns result data dict.
    """
    document = db.documents.find_one({"_id": document_id}) if document_id else None
    if document_id and not document:
        raise ValueError(f"Document {document_id} not found")

    action_type = workflow.get("action_type", "")
    action_config = workflow.get("action_config") or {}

    if action_type == "send_email":
        return _action_send_email(document, action_config)
    elif action_type == "create_tag":
        return _action_create_tag(document, action_config, db)
    elif action_type == "generate_report":
        return _action_generate_report(document, action_config)
    elif action_type == "webhook":
        return _action_webhook(document, action_config)
    elif action_type == "slack":
        return _action_slack(document, action_config)
    else:
        return {"status": "skipped", "reason": f"Unknown action type: {action_type}"}


def check_workflow_triggers(document: dict, db: Database):
    """Check all active workflows and trigger matching ones after document processing."""
    workflows = list(db.workflows.find({
        "user_id": document.get("user_id"),
        "is_active": True,
    }))

    for wf in workflows:
        if _should_trigger(wf, document):
            try:
                execute_workflow(wf, document["_id"], db)
                db.workflows.update_one({"_id": wf["_id"]}, {"$inc": {"run_count": 1}})
            except Exception as e:
                print(f"Workflow {wf['_id']} failed: {e}")


def _should_trigger(workflow: dict, document: dict) -> bool:
    """Check if a workflow should be triggered for this document."""
    condition = workflow.get("trigger_condition") or {}

    if workflow.get("trigger_type") == "document_uploaded":
        required_type = condition.get("document_type")
        if required_type and document.get("document_type") != required_type:
            return False
        return True

    elif workflow.get("trigger_type") == "field_value":
        field = condition.get("field")
        if not field or not document.get("extracted_fields"):
            return False
        return field in document["extracted_fields"]

    return False


# ─── Action Implementations ───

def _action_send_email(document: dict, config: dict) -> Dict[str, Any]:
    """Send an email notification about a document."""
    to_email = config.get("to", "")
    template = config.get("template", "default")
    subject = f"DocuSmart Alert: {document.get('document_type')} - {document.get('original_filename')}"

    print(f"[EMAIL] To: {to_email} | Subject: {subject} | Template: {template}")

    return {
        "action": "send_email",
        "to": to_email,
        "subject": subject,
        "status": "sent",
    }


def _action_create_tag(document: dict, config: dict, db: Database) -> Dict[str, Any]:
    """Auto-tag a document based on its content."""
    tags_to_add = config.get("tags", [])

    fields = document.get("extracted_fields") or {}
    if fields.get("vendor_name"):
        tags_to_add.append(f"vendor-{fields['vendor_name'].lower().replace(' ', '-')}")
    if fields.get("total_amount") and float(fields["total_amount"]) > 500:
        tags_to_add.append("over-$500")
    if document.get("document_type"):
        tags_to_add.append(document["document_type"])

    existing_tags = document.get("tags") or []
    new_tags = list(set(existing_tags + tags_to_add))
    db.documents.update_one({"_id": document["_id"]}, {"$set": {"tags": new_tags}})

    return {
        "action": "create_tag",
        "tags_added": tags_to_add,
        "status": "completed",
    }


def _action_generate_report(document: dict, config: dict) -> Dict[str, Any]:
    """Generate a report for the document."""
    return {
        "action": "generate_report",
        "document_id": document["_id"],
        "status": "report_queued",
    }


def _action_webhook(document: dict, config: dict) -> Dict[str, Any]:
    """Send document data to a webhook URL."""
    url = config.get("url", "")
    print(f"[WEBHOOK] POST to {url} with document {document['_id']}")

    return {
        "action": "webhook",
        "url": url,
        "status": "sent",
    }


def _action_slack(document: dict, config: dict) -> Dict[str, Any]:
    """Send a Slack notification."""
    channel = config.get("channel", "#general")
    message = f"New {document.get('document_type')}: {document.get('original_filename')}"
    print(f"[SLACK] #{channel}: {message}")

    return {
        "action": "slack",
        "channel": channel,
        "message": message,
        "status": "sent",
    }
